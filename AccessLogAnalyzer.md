# Access Log Analyzer - 설계 문서

이 문서는 Access Log Analyzer의 상세한 설계 및 아키텍처를 설명합니다. 기본 사용법과 예제는 `README.md`와 `USAGE_EXAMPLES.md`를 참조하세요.
...
```json
{
  "mcpServers": {
    "access-log-analyzer": {
      "command": "python3",
      "args": [
        "/path/to/alb_accesslog_analyzer/mcp_server.py"
      ],
      "env": {
        "PYTHONPATH": "/path/to/alb_accesslog_analyzer"
      }
    }
  }
}
